...
AC_DEFUN([AC_CHECK_SQLITE3], [
  ...
  if test "x$ac_sqlite3" = "xyes"; then
      ...
      if test "$ac_sqlite3_libdir" = "no"; then
          ...
          dnl Debian/Ubuntu multiarch puts libsqlite3.so in /usr/lib/<triplet>,
          dnl which the fixed list above predates. Look there first when we can.
          sqlite3_multiarch=`$CC -print-multiarch 2>/dev/null`
          if test -n "$sqlite3_multiarch"; then
              sqlite3_libdirs="/usr/lib/$sqlite3_multiarch /usr/local/lib/$sqlite3_multiarch $sqlite3_libdirs"
          fi
