 ...
 *
 * $Id: res.c 29221 2015-12-10 19:54:15Z androsyn $
 * 
...
/* Both of the generators below feed DNS anti-spoofing: the query id and the
 * source port an answer has to match. If the RNG ever fails we must not carry
 * on with whatever happened to be on the stack - that is both predictable and,
 * in generate_random_port()'s case, a potential spin forever on a value that
 * never changes. There is no safe weaker fallback for this, so die loudly and
 * let the ircd restart the resolver, as it does for any other helper fault.
 */
static void
random_or_die(void *buf, size_t length)
{
	if(!rb_get_pseudo_random(buf, length))
		abort();
}
...
generate_random_port(void)
{
	...
	while(1)
	{
		random_or_die(&port, sizeof(port));
		if(port > 1024)
			break;
	}
	return (int)port;
