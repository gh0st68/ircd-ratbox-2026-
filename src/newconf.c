 ...
 *  USA
 *
 *  $Id: newconf.c 29291 2016-01-13 18:07:49Z androsyn $               
 * 
 */
...
conf_set_end_operator(conf_t * conf)
{
	...
	RB_DLINK_FOREACH_SAFE(ptr, next, t_oper_list.head)
	{
		...
		if(t_oper->rsa_pubkey_file != NULL)
		{
			 ...
			 * ("BEGIN PUBLIC KEY") files the old RSA-specific call did,
			 * so existing operator key files keep working.
			 */
			tmp_oper->rsa_pubkey = PEM_read_bio_PUBKEY(file, NULL, NULL, NULL);
			BIO_free(file);
			...
			if(EVP_PKEY_base_id(tmp_oper->rsa_pubkey) != EVP_PKEY_RSA)
			{
				conf_report_error_nl
					("operator block for %s at %s:%d -- rsa_public_key_file is not an RSA key",
					 tmp_oper->name, conf->filename, conf->line);
				EVP_PKEY_free(tmp_oper->rsa_pubkey);
				tmp_oper->rsa_pubkey = NULL;
				return;
			}
		}
#endif
