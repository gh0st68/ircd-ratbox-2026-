...
rb_init_ssl(void)
{
	...
	{
		/* Disable the obsolete protocols, make the client use our settings */
		SSL_CTX_set_options(ssl_server_ctx,
...
rb_setup_ssl_server(const char *cert, const char *keyfile, const char *dhfile)
{
	...
	/* Disable everything below TLS 1.2, make the client use our settings */
	tls_opts |= SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 |
...
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
	/* OpenSSL 1.1.0 and later negotiate the ECDH curve automatically, and the
	 * EC_KEY / SSL_CTX_set_tmp_ecdh() pair this used to need is deprecated in
	 * 3.x. Pinning a single curve here would only narrow what we can agree on.
	 */
	(void)named_curve;
...
rb_get_random(void *buf, size_t length)
{
	...
	/* RAND_bytes() returns 1 on success; 0 means it failed and -1 means the
	 * RNG is not supported at all. Anything but 1 leaves buf untrustworthy,
	 * so normalise to a plain 1/0 - callers only test for truth.
	 */
	if((ret = RAND_bytes(buf, length)) != 1)
	{
...
{
	/* RAND_pseudo_bytes() is deprecated and, since OpenSSL 1.1.0, no weaker
	 * than RAND_bytes() anyway. Same 1-means-success contract as above.
	 */
...
rb_get_ssl_info(char *buf, size_t len)
{
	rb_snprintf(buf, len, "Using SSL: %s compiled: 0x%lx, library 0x%lx",
		    OpenSSL_version(OPENSSL_VERSION), OPENSSL_VERSION_NUMBER,
		    OpenSSL_version_num());
}
