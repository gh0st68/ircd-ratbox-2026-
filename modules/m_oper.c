 ...
 *  USA
 *
 *  $Id: m_oper.c 27437 2013-02-02 20:33:47Z jilles $
 */
...
DECLARE_MODULE_AV1(oper, NULL, NULL, oper_clist, NULL, NULL, "$Revision: 27437 $");
...
static int generate_challenge(uint8_t **r_challenge, uint8_t **r_response, EVP_PKEY * pkey);
...
static int
generate_challenge(uint8_t **r_challenge, uint8_t **r_response, EVP_PKEY * pkey)
{
	EVP_MD_CTX *ctx = NULL;
	EVP_PKEY_CTX *pctx = NULL;
	uint8_t secret[CHALLENGE_SECRET_LENGTH], *tmp = NULL;
	...
	/* The low-level SHA1_*() and RSA_public_encrypt() calls this used to make
	 * are deprecated as of OpenSSL 3.0; the EVP equivalents below produce a
	 * byte-identical challenge, so existing oper clients are unaffected.
	 * Everything stays in locals until it has all succeeded, so a failure
	 * leaves the caller's *r_challenge / *r_response exactly as they were.
	 */
	if(rb_get_random(secret, CHALLENGE_SECRET_LENGTH))
	{
		...
		if(EVP_DigestInit_ex(ctx, EVP_sha1(), NULL) != 1)
			goto fail;
		if(EVP_DigestUpdate(ctx, secret, CHALLENGE_SECRET_LENGTH) != 1)
			goto fail;

		response = rb_malloc(SHA_DIGEST_LENGTH);
		if(EVP_DigestFinal_ex(ctx, response, &digest_len) != 1
		...
		if(EVP_PKEY_encrypt(pctx, NULL, &length, secret, CHALLENGE_SECRET_LENGTH) != 1
		   || length == 0 || length > INT_MAX)
			goto fail;

		tmp = rb_malloc(length);
		if(EVP_PKEY_encrypt(pctx, tmp, &length, secret, CHALLENGE_SECRET_LENGTH) != 1)
			goto fail;

		EVP_PKEY_CTX_free(pctx);
		*r_challenge = rb_base64_encode(tmp, (int)length);
		*r_response = response;
		rb_free(tmp);
		return 0;
	}
